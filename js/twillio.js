$(function () {
  ...
  const callButton = document.getElementById("dialer_call_btn_td");
  ...
  const phoneNumberInput = document.getElementById("dialer_number");
  const incomingPhoneNumberEl = document.getElementById("incoming-number");
  const startupButton = document.getElementById("call-button");

  let device;
  let token;
  let call_time=0;
  let call_start_time=0;
  ...
  async function startupClient() {
    ...
    try {
      const data = await $.getJSON("https://call.itsinfotech.com/token.php");
      log("Got a token.");
      token = data.token;
  ...
  async function makeOutgoingCall() {
    ...
    if (device) {
     ...
     call_api({cmd: "new_outgoing_call", number: params.To});

     let dat = new Date();
     call_start_time = dat.getTime();
  ...
  function updateUIDisconnectedOutgoingCall() {
    ...
    let dat = new Date();
    call_time = dat.getTime() - call_start_time;

    call_api({cmd: "call_time", number: phoneNumberInput.value, time: call_time});
