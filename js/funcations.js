function renderTriggers(){
    ...
    for (let x in triggers) {
        if(triggers[x])
        txt += '<button type="button" class="btn btn-light m-1 trigger_btn" onclick="editTrigger('+x+')">'+triggers[x]['title']+'</button>';
    }
    document.getElementById("created_trigger").innerHTML = txt;
}function editTrigger(index){
    $('#trigger_index').val(index);
    $('#choose_trigger').val(triggers[index]['value']);
    $('#datatriggerindex').val(triggers[index]['id']);
    $('#selected_trigger_value').val(triggers[index]['title']);
    if(triggers[index]['value'] == "Appointment Status"){
        $('#addtriggerModal').find('#filters_section').removeClass('d-none');
        $('select[name=filters]').val(triggers[index]['filters_value']);
    }else{
        $('#addtriggerModal').find('#filters_section').addClass('d-none');
        $('select[name=filters]').val('');
    }
    $('#addtriggerModal').modal('show');
    $('#addtriggerModal').find('.deletetrigger').removeClass('d-none');
    $('#addtriggerModal').find('.deletetrigger').attr('data-id',index);
}function checkisTrigger(index,value){
    for (let x in triggers) {
        if(x != index && triggers[x]['value'] == value){
            return true;
        }
    }
    return false;
        ...
        if(actionss[x]){
            ...
            if(actionss[x]['modal_id'] == 'conditionsactionModal'){
                ...
                for (let c in actionss[x]['formdt']['branches']) {
                    // console.log(actionss[x]['formdt']['branches'][c]);
                    acttion +=`<div class="col-3">
                            <button type="button" class="btn btn-primary btn-branchactive d-block mt-4 m-auto btn-acction">
                                <i class="fa fa-envelopes"></i> ${actionss[x]['formdt']['branches'][c]}
                            </button>
                            <button type="button" class="btn btn-light d-block mt-4 m-auto add_plus_action" onclick="acction_post(${parseInt(x)+1},${x})" data-id="${parseInt(x)+1}" data-bs-toggle="modal" data-bs-target="#addactionModal">
...
}function saveinstagramactionModalForm(){
    let call_msg = $('textarea[name=call_msg]').val();
    return {
        call_msg:call_msg
    };
...
}function saveaddcontacttagactionModalForm(){
    let add_contact_tags = $('input[name=add_contact_tags]').val();
    return {
        add_contact_tags:add_contact_tags
    };
}function saveremovecontacttagactionModalForm(){
    let remove_contact_tags = $('input[name=remove_contact_tags]').val();
    return {
