...
$('.select_action').click(function(){
    ...
    if(actionmodal == "#greetingactionModal")
...
$('.save_action').click(function(){
    ...
    let actiontitle = $('#'+targetmodal).find('.action_name').val();
    ...
    if(targetmodal == "conditionsactionModal"){
        ...
        // console.log(formdt);
        if(targetmodal == "editconditionsactionModal"){
            actiontitle= "Press "+actionname
        }
            ...
            <div class="row p-3">
                <div class="col-md-11">
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control" value="${c}" name="branches[]" />
                    </div>
                </div>
            </div>
...
$('#greeting-media-upload').change(function(e) {
    var formData = new FormData();
    $('#savegreenting').attr('disabled',true);
    $('#savegreenting').text('uploading');

    formData.append("media_file", e.target.files[0]);
    formData.append("cmd", "upload_media");
    $.ajax({
        url: 'server.php',
        type: 'POST',
        data: formData,
        ...
        success: function (result) {
            ...
            $('#savegreenting').removeAttr('disabled');
            $('#savegreenting').text('save');
            console.log(result);
            if(result)
                $('input[name=greeting_media]').val(result);
        }
    });
});