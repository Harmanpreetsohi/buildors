...
$workflow_data = mysqli_query($link,"select * from ivr_rsponses where id='".$_REQUEST['flowid']."'");
...
?>
        ...
        <div class="container-fluid justify-content-between">
            <div style="display: flex;" >
                <div class="form-group">
                    ...
                    <input class="form-control" type="text" name="ivr_title"
                        value="<?= $workflow_data['name'] ?>" placeholder="Write Response Name...">
                </div>
                <div class="form-group" style="min-width: 228px;margin-left: 14px;" >
                    ...
                    <select class="form-control form-select" type="text" name="assigned_number" required >
                        ...
                        <option selected value="<?= $workflow_data['assigned_number'] ?>" ><?= $workflow_data['assigned_number'] ?></option>
                        <?php
                        $query = mysqli_query($link,"SELECT twilio_numbers.phone_number FROM `twilio_numbers`  LEFT JOIN ivr_rsponses on ivr_rsponses.assigned_number = twilio_numbers.phone_number WHERE ivr_rsponses.assigned_number IS NULL AND twilio_numbers.user_id=".$_SESSION['user_id']);
    ...
    <script>
        var treeData = JSON.parse('<?= $workflow_data['treeData'] ?>');
        var actionss = JSON.parse('<?= str_replace("\n", '', $workflow_data['actionss']) ?>');
        ...
        $('#save_ivr').click(function(){
            ...
            if(ivr_title){
                ...
                data.append("cmd","save_ivr_response");
